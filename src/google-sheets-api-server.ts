  ...
  loadToken(): TokenData | null {
    ...
    if (process.env.GOOGLE_ACCESS_TOKEN) {
      try {
        ...
        console.log('✅ Token loaded from GOOGLE_ACCESS_TOKEN environment variable');
        return token;
      } catch (err) {
    ...
    try {
      ...
      this.cache.set(this.CACHE_KEY, token);
      console.log('✅ Token loaded from token.json file (cached)');
      return token;
    } catch (err) {
  ...
  try {
    ...
    if (useMultiUser && tokenStorage) {
      try {
        ...
        if (userEmail) {
          ...
          if (!mcpToken || mcpToken === 'default') {
            ...
            console.log(`🔑 Auto-generated MCP token for user: ${userId}`);
          }
          ...
          if (created) {
            console.log(`✅ ${autoGeneratedToken ? 'Auto-created' : 'Created'} user with MCP token: ${userId} (${userEmail})`);
          } else {
          ...
          await tokenStorage.storeToken(userId, tokens as TokenData);
          console.log(`✅ Token stored for user: ${userId} (no email available)`);
        }
      ...
      try {
        ...
        console.log('✅ Token also saved to token.json for backward compatibility');
      } catch (fileErr) {
