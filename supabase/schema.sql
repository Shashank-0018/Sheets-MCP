...
CREATE TABLE IF NOT EXISTS mcp_users (
    ...
    mcp_token_hash VARCHAR(255), -- Hashed MCP token for authentication (nullable - can be linked later)
...
-- Partial unique index: ensures uniqueness only for non-NULL mcp_token_hash values
-- This allows multiple users without MCP tokens while enforcing uniqueness for actual tokens
